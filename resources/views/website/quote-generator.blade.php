      ...
      <div class="container">
        <div class="tcw-quote-hero-grid">
          <div class="tcw-quote-copy wow fadeInUp" data-wow-duration="1s">
            <span class="tcw-detail-eyebrow">Dynamic Quote Generator</span>
            <h1>Build an instant project estimate with a premium proposal.</h1>
            <p>Select a service, choose your budget, share your requirements, and get a polished quotation instantly.</p>
          </div>
      ...
      <div class="container">
        <form action="{{ route('website.quote-generator.store') }}" method="POST" class="tcw-quote-form wow fadeInUp" data-wow-duration="1s">
            ...
            <label>
              ...
              <select name="service_id" required>
                ...
                @foreach($services as $service)
                  <option value="{{ $service->id }}" @selected(old('service_id') == $service->id)>{{ $service->service_title }}</option>
                @endforeach
              </select>
            </label>
            <label>
          ...
          <button type="submit" class="tcw-quote-submit" data-quote-submit>
            Submit & Generate PDF Proposal <i class="fas fa-file-invoice-dollar"></i>
          </button>
        </form>
      </div>
    </section>
